...
for n = 1:length(beta)
    for l = 1: 10
        ...
        log_weight(1,:)=-0.5*(yy(1).^2./((beta(n)^2)*exp(x_estimate(1,:))))...
            -0.5*log(2*pi*exp((beta(n)^2)*x_estimate(1,:)))-0.5*x_estimate(1,:).^2/sigma^2;        % logarithm weight
...
title('Prob1(b): Likelihood box plot for different values of \beta = 0 to 2, N= 500, T = 500');
xlabel('\beta');ylabel('likelihood');


%% HW5_Prob1_(b) N = 100, T = 500
clear all;
...
figure;
box = boxplot(likelihood', beta); grid on;
title('Prob1(b): Likelihood box plot for different values of \beta = 0 to 2, N = 100, T = 500');
xlabel('\beta');ylabel('likelihood');


%% HW5_Prob1_(b) N = 500, T = 100
clear all;
...
figure;
box = boxplot(likelihood', beta); grid on;
title('Prob1(b): Likelihood box plot for different values of \beta = 0 to 2, N = 500, T = 100');
xlabel('\beta');ylabel('likelihood');