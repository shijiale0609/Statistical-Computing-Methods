...
import pandas as pd 
data = pd.read_csv('/Users/shijiale1995/Box Sync/Courses/Statistics Computing/HW2/Assignment-2_Final/Python/data.csv',header=None)
data = np.array(data)
y = data[:,0]
n = data[:,1]
print (y)
...
def log_likelihood(alpha,beta,y,n):
	...
	# INSERT CODE BELOW
	##############################################################	
    log_likelihood+= gammaln(alpha+beta)+gammaln(alpha+Y)+gammaln(beta+N-Y)-gammaln(alpha)-gammaln(beta)-gammaln(alpha+beta+N)
	##############################################################
	# INSERT CODE ABOVE
	##############################################################
