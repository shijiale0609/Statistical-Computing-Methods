...
% covoption = 1 means isotropic covariances
...
[dimens,npoints] = size(y); % dimens=2
...
while(cont)    % this inner loop is the component-wise EM algorithm with the
   ...
   while comp <= k
            ...
            if (covoption == 0)|(covoption == 2)
               ...
               estcov(:,:,comp) = normalize*diag(sum(aux.*y,2)) - diag(estmu(:,comp).^2) ; % this is for covoption ==1
               % modify the covariance matrix, change it from diag to
               % isotropic
               t1 = estcov(1,1,comp);
               t2 = estcov(2,2,comp);
               t = (t1+t2)/2;
...
%csvwrite('dl.csv',dl);
%csvwrite('transitions1.csv',transitions1);
%csvwrite('transitions2.csv',transitions2);
...
% the best covariance for each number of compoments of a mixture of
% Gaussian
index = [transitions1,transitions2]
indexs = sort(index)
covi = []
for i=1:length(indexs)
    covi = [covi,dl(indexs(i))];
end
covi = [covi,dl(length(dl))]
figure

%the array of number of components of a mixture of Gaussian
gamount = kmin:kmax
plot(gamount,fliplr(covi),'r-o')
%scatter(gamount,fliplr(covi))
xlabel('the number of components of a mixture of Gaussian')
ylabel('log-likelihood')
figure(thefig)
